...
| `WebPush__PublicKey` | пусто | Публичный VAPID-ключ, который получают браузеры при подписке |
| `WebPush__PrivateKey` | пусто | Приватный VAPID-ключ, доступный только серверу |
| `WebPush__Subject` | пусто | Контактный URI для VAPID (`mailto:` или `https:`) |

### Web Push

Web Push необязателен. Если три переменные пусты, Merito запускается как обычно, а браузерные
push-уведомления остаются отключенными. Чтобы включить их, один раз создайте VAPID-пару и постоянно
используйте ее для этой установки. Если установлены Node.js и `npx`, выполните:

```bash
npx --yes web-push generate-vapid-keys --json
```

Если установлен только Docker, запустите тот же генератор во временном контейнере:

```bash
docker run --rm node:lts-alpine npx --yes web-push generate-vapid-keys --json
```

Перенесите полученные значения в `.env` и укажите контакт владельца сервера:
...
`WebPush__Subject` может быть адресом `mailto:` или ссылкой `https:`. Публичный ключ специально
передается браузерам. Приватный ключ должен оставаться секретным: храните его только в `.env` или в
хранилище секретов и никогда не добавляйте в Git. Создавайте пару только один раз; после ее замены
существующие подписки браузеров перестанут работать, и пользователям придется подписаться заново.
